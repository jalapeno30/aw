...
function Payment($http, ENV) {
    ...
    self.purchaseBet = function(token, userId, orderIds){
        var url = ENV.apiEndpoint + '/payment/purchaseBet?token=' + token;   
        var date = new Date();
          var options = {
...
//    };
 ...
 //Step 2A send new card data
    ...
    self.continueExistingCard = function(token, cardId, transId, userId){
        var url = ENV.apiEndpoint + '/payment/continueExistingCard?token=' + token;     
        var data = {
          cardId: cardId,
          transId: transId,
          userId: userId
    ...
    self.updateStatus = function(token, userId, transId, status){
        var url = ENV.apiEndpoint + '/payment/status?token=' + token;   
        var data = {
          userId: userId,
          transId: transId,
          status: status
    ...
    self.existingCardDetails = function(token, userId){
        var url = ENV.apiEndpoint + '/payment/existingCardDetails?token=' + token + '&userId=' +userId;         
        var data = {
          userId: userId
        };
        //this is temp
    ...
    self.deleteCard = function(token, userId, cardId){
        var url = ENV.apiEndpoint + '/payment/deleteCard?token=' + token;   
        var data = {
          userId: userId,
          cardId: cardId
        };
