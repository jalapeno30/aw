...
angular.module('lotteryApp')
  .controller('PaymentTestCtrl', function (User, Orders, $scope, Payment) {
    ...
    self.cost;
    self.status = 0;
//   transactionId should be coming from step0 (step0Back). this now works
    self.transId;
//    should be coming from form, not yet functioonal
...
//  Not yet functional. Send card id after fetching existing cards
    self.cardId = 1;
//    temporary
    self.step0Out;
    self.step0Back;
    ...
    self.step0 = function (){ 
        Payment.purchaseBetT(self.token, self.userId, self.orderIds).then(function(response){
...
//  };
    ...
    self.step2A = function(){
        Payment.newCardDetailsT(self.token, self.jsonCard(true), self.transId, self.userId).then(function(response){
    ...
    self.step2B = function(){
        Payment.continueExistingCardT(self.token, self.cardId, self.transId, self.userId).then(function(response){
    ...
    self.step3 = function(status){
        var status = status;
        Payment.updateStatusT(self.token, self.userId, self.transId, status).then(function(response){
    ...
    self.existingCardDetails = function(){
        Payment.existingCardDetailsT(self.token, self.userId).then(function(response){
